...
class summoner_pro_player(models.Model):
    _name = 'summoner.pro.player'
    _inherit = ['mail.thread']

    name = fields.Char('Name')
    pro_team_id = fields.Many2one('summoner.pro.team','Pro Team')
    role  = fields.Selection([('top','Top'),
                              ('jungle','Jungle'),
                              ('mid','Mid'),
                              ('adc','AD Carry'),
                              ('support','Support')])
...
class summoner_pro_team(models.Model):
    ...
    pro_player_ids = fields.One2many('summoner.pro.player','pro_team_id', 'Pro Players')
